...
function signedPointText(value: number): string {
  if (value > 0) {
...
export default async function SimulatorPage({
  ...
  const departmentNameMap = new Map(
    input.departments.map((department) => [department.id, department.name]),
  );
  const baselineMap = new Map(
    baseline.reasons.map((item) => [item.candidateId, item]),
  );
  ...
  const departmentRows = input.departments.map((department) => {
    ...
    for (const candidate of input.candidates) {
      ...
      if (baselineAssignedDepartment === department.id) {
        const rank =
          candidate.preferences.findIndex(
            (deptId) => deptId === department.id,
          ) + 1 || null;
    ...
    const baselineTotal = Object.values(baselineBuckets).reduce(
      (sum, value) => sum + value,
      0,
    );
    const daTotal = Object.values(daBuckets).reduce(
      (sum, value) => sum + value,
      0,
    );
    ...
    .map((candidate) => {
      ...
      return {
        ...
        baselineAssignmentName: baselineAssignment
          ...
          : "未配属",
        daAssignmentName: daAssignment
          ? (departmentNameMap.get(daAssignment) ?? daAssignment)
          : "未配属",
        baselineReason:
          baselineMap.get(candidate.id)?.summary ?? "理由データなし",
  ...
  const top1Diff =
    (da.metrics.firstChoiceRate - baseline.metrics.firstChoiceRate) * 100;
  const blockingDiff =
    da.metrics.blockingPairs - baseline.metrics.blockingPairs;
  ...
  const histogramDiffRows = baseline.satisfactionHistogram.map((bin, index) => {
    const daBin = da.satisfactionHistogram[index];
    const daValue = daBin?.value ?? 0;
    const diff = daValue - bin.value;
    const baselineRate =
      input.config.candidateCount === 0
        ? 0
        : (bin.value / input.config.candidateCount) * 100;
    const daRate =
      input.config.candidateCount === 0
        ? 0
        : (daValue / input.config.candidateCount) * 100;
    return {
      label: bin.label,
      baselineValue: bin.value,
      daValue,
      diff,
      baselineRate,
      daRate,
      rateDiff: daRate - baselineRate,
      daBetter: bin.label === "未配属" ? diff < 0 : diff > 0,
    };
  });
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
              <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-700">
                N {input.config.candidateCount}
              </span>
              <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-700">
                M {input.config.departmentCount}
              </span>
              <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-700">
      ...
      <div className="space-y-4">
        <section className="surface-card rounded-3xl brand-gradient-soft p-4">
          ...
          <p className="mt-1 text-xs leading-6 text-blue-900/90">
            <span className="font-semibold">Baseline（現在方式）</span>
            は、部署ごとに候補者を見て定員を順に埋めるシンプルな割り当てです。
          </p>
          <p className="mt-2 text-xs leading-6 text-blue-900/90">
            <span className="font-semibold">DA</span>
            は、新卒が希望順で応募し、部署はいったん保留しながらより合う人を残していく方式です。
            <span className="font-semibold"> 左=Baseline / 右=DA</span>
            で同条件比較します。
          </p>
        </section>
        ...
        <section className="rounded-3xl border border-blue-800 bg-blue-900 p-4 text-white shadow-lg shadow-blue-200/50">
          ...
          <p className="mt-1 text-lg font-bold">
            ...
            {top1Diff.toFixed(1)}pt 変化、ブロッキングペアを {blockingDiff}{" "}
            件変化
          </p>
        ...
        <section className="surface-card rounded-3xl p-4">
          <h2 className="text-sm font-semibold text-slate-800">
            メイン比較: 満足度分布（左: Baseline / 右: DA）
          </h2>
          <p className="mt-1 text-xs text-slate-600">
            改善率を見るときは、まずここを見ると全体像を直感的につかめます。
          </p>
          <div className="mt-4 space-y-3">
            {histogramDiffRows.map((row) => (
              <article
                key={`hist-main-${row.label}`}
              ...
              >
                <div className="flex items-center justify-between text-xs">
                  ...
                  >
                    DA差分 {signedCount(row.diff)}人（
                    {signedPointText(row.rateDiff)}）
                  </p>
                </div>
                <div className="mt-2 grid gap-2 md:grid-cols-2">
                  <div>
                    ...
                    <div className="h-3 overflow-hidden rounded-full bg-slate-100">
                      <div
                        ...
                        style={{
                          width: `${(row.baselineValue / input.config.candidateCount) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                  <div>
                    <div className="mb-1 flex items-center justify-between text-[11px] text-slate-600">
                      ...
                      <span>
                        {row.daValue}人（
                        {percentText(row.daValue, input.config.candidateCount)}
                        ）
                      </span>
                    </div>
                    <div className="h-3 overflow-hidden rounded-full bg-slate-100">
                      <div
                        ...
                        style={{
                          width: `${(row.daValue / input.config.candidateCount) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <section className="surface-card rounded-3xl p-4">
          <h2 className="text-sm font-semibold text-slate-800">
            補助指標（左: Baseline / 右: DA）
          </h2>
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <article className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="mb-3 inline-flex rounded-full bg-slate-800 px-3 py-1 text-xs font-bold text-white">
                Baseline（現在方式）
              </p>
                ...
                {kpiLabels.map((kpi) => (
                  ...
                  >
                    ...
                    <span className="font-semibold text-slate-900">
                      {formatMetricValue(baseline.metrics[kpi.key], kpi.unit)}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="mb-3 inline-flex rounded-full bg-blue-700 px-3 py-1 text-xs font-bold text-white">
                DA
              </p>
              <div className="space-y-2">
                {kpiLabels.map((kpi) => (
                  <div
                    key={`d-${kpi.key}`}
                    className="flex items-center justify-between rounded-xl bg-blue-50 px-3 py-2 text-sm"
                  >
                    <span className="text-slate-600">{kpi.label}</span>
                    <span className="font-semibold text-slate-900">
                      {formatMetricValue(da.metrics[kpi.key], kpi.unit)}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <section className="surface-card rounded-3xl p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {RANK_BUCKETS.map((bucket) => (
              ...
              >
                <span
                  className={`inline-block h-2.5 w-2.5 rounded-full ${bucket.color}`}
                />
                ...
                >
                  ...
                  <div className="mt-2 grid gap-2 lg:grid-cols-2">
                    <div className="rounded-xl bg-slate-50 p-2">
                      ...
                      <p className="mt-1 text-xs text-slate-600">
                        配属: {row.baselineAssignmentName}
                      </p>
                      <p className="mt-1 text-xs text-slate-600">
                        理由: {row.baselineReason}
                      </p>
                    </div>
                    <div className="rounded-xl bg-blue-50 p-2">
                      <p className="text-xs font-bold text-blue-700">DA</p>
                      <p className="mt-1 text-xs text-slate-600">
                        配属: {row.daAssignmentName}
                      </p>
                      <p className="mt-1 text-xs text-slate-600">
                        理由: {row.daReason}
                      </p>
